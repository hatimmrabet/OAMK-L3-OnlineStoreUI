...
const ModifyItem = (props) => {
    ...
    let OldItem = props.route.params.item;
    const [itemTitle, setItemTitle] = useState(OldItem.title);
    const [itemDescription, setItemDescription] = useState(OldItem.description);
    const [itemCategory, setItemCategory] = useState(OldItem.category);
    const [itemCity, setItemCity] = useState(OldItem.city);
    const [itemContryCode, setItemContryCode] = useState(OldItem.contryCode);
    const [itemPrice, setItemPrice] = useState(OldItem.price.toString());
    const [itemDelivery, setItemDelivery] = useState(OldItem.delivery);
                ...
                <View>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-around', marginBottom:25 }}>
                        <TouchableHighlight
                            ...
                            onPress={() =>
                                props.onItemModify(OldItem.id, itemTitle, itemDescription, itemPrice, itemCategory, itemCity, itemContryCode, itemDelivery)}>
                            <View>
